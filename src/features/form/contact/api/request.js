...
export async function createRequest(lang, formData, tableName, pathToRevalidate) {
  try {
    // 유효성 검사
    const validationResult = formScheme(lang).safeParse(formData);
    ...
    const { error, data: insertedData } = await query.select("*").single();
    ...
    const sendAlimMailResult = await sendAlimMail({
      tableName,
      mailSubject: `[고객문의접수] ${insertedData.name}님의 문의가 접수되었습니다.`,
      mailBody: getMailBody(lang, insertedData),
    });
...
function getMailBody(lang, data) {
  return `
  <!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 0; background-color: #f7f7f7; font-family: 'Apple SD Gothic Neo', 'Noto Sans KR', 'Malgun Gothic', sans-serif; color: #333333; line-height: 1.5;">
  ...
  <div style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
    ...
    <div style="padding: 25px 30px 5px; border-bottom: 1px solid #e2e8f0;">
      <h1 style="margin: 0; color: #1e293b; font-size: 22px; font-weight: 700;">새 문의가 접수되었습니다</h1>
    </div>
    ...
    <div style="padding: 20px 30px;">
      <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
        <tr>
          <td style="padding: 8px 0; color: #64748b; width: 90px; vertical-align: top; border-bottom: 1px solid #e2e8f0;">문의구분</td>
          <td style="padding: 8px 0; font-weight: 500; border-bottom: 1px solid #e2e8f0;">
            <span style="background-color: #dbeafe; color: #2563eb; padding: 2px 8px; border-radius: 4px; font-size: 13px;">${data.category}</span>
          </td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #64748b; vertical-align: top; border-bottom: 1px solid #e2e8f0;">제품</td>
          <td style="padding: 8px 0; font-weight: 500; border-bottom: 1px solid #e2e8f0;">${data.product.toUpperCase()}</td>
        </tr>
        <tr>
        ...
        ${
          lang === "en"
            ? `
              <tr>
                <td style="padding: 8px 0; color: #64748b; vertical-align: top; border-bottom: 1px solid #e2e8f0;">국가</td>
                <td style="padding: 8px 0; border-bottom: 1px solid #e2e8f0;">${data.country}</td>
              </tr>
        `
            : ""
        }
        <tr>
          <td style="padding: 8px 0; color: #64748b; vertical-align: top; border-bottom: 1px solid #e2e8f0;">${lang === "ko" ? "회사명" : "회사 URL"}</td>
      ...
      <div style="margin-top: 20px; background-color: #f8fafc; border-radius: 6px; padding: 20px;">
        <h3 style="margin: 0 0 10px 0; font-size: 15px; color: #64748b; font-weight: 600;">문의 내용</h3>
