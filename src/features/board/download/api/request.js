...
import { downloadRequestFormScheme } from "../download-request-form.schema";
...
export async function createRequest(lang, formData, tableName, pathToRevalidate) {
  try {
    // 유효성 검사
    const validationResult = downloadRequestFormScheme(lang).safeParse(formData);
    if (!validationResult.success) {
      throw validationResult.error;
    }

    const { category, title, ...toInsertData } = validationResult.data;
    ...
    if (lang === "ko") {
      const sendAlimMailResult = await sendAlimMail({
        tableName,
        mailSubject: `[다운로드요청접수] ${toInsertData.name}님의 다운로드 요청이 접수되었습니다.`,
...
<body style="margin: 0; padding: 0; background-color: #f7f7f7; font-family: 'Apple SD Gothic Neo', 'Noto Sans KR', 'Malgun Gothic', sans-serif; color: #333333; line-height: 1.5;">
  ...
  <div style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
    ...
    <div style="padding: 25px 30px 5px; border-bottom: 1px solid #e2e8f0;">
      <h1 style="margin: 0; color: #1e293b; font-size: 22px; font-weight: 700;">다운로드 요청이 접수되었습니다</h1>
    </div>
    ...
    <div style="padding: 20px 30px;">
      <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
        <tr>
          <td style="padding: 8px 0; color: #64748b; width: 90px; vertical-align: top; border-bottom: 1px solid #e2e8f0;">요청자료</td>
          <td style="padding: 8px 0; font-weight: 500; border-bottom: 1px solid #e2e8f0;">
            <span style="background-color: #dbeafe; color: #2563eb; padding: 2px 8px; border-radius: 4px; font-size: 13px;">${data?.category?.toUpperCase()}</span>
            ${data?.title}
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding: 8px 0; color: #64748b; vertical-align: top; border-bottom: 1px solid #e2e8f0;">회사</td>
          <td style="padding: 8px 0; font-weight: 500; border-bottom: 1px solid #e2e8f0;">${data.company}</td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #64748b; vertical-align: top; border-bottom: 1px solid #e2e8f0;">부서</td>
          <td style="padding: 8px 0; font-weight: 500; border-bottom: 1px solid #e2e8f0;">${data.department}</td>
        </tr>
      </table>
