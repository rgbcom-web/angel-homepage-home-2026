...
export function AngelStoryView({ data, prevData, nextData }) {
  ...
  return (
    <Container width="narrower" className={cn("divide-y divide-dd-gray-light/50")}>
      ...
      <div className={cn("flex justify-end pt-8")}>
        <BoardViewListButton isFixed={data?.fixed} />
      </div>
    </Container>
  );
}
...
function BoardViewHeader({ data }) {
  const { title, created_at, published_at, views, category } = data;
    ...
    <div
      ...
      {category && (
        <div className={cn("flex justify-center")}>
          <Badge variant={category.color} className={cn("text-lg tablet:text-sm")}>
            {category.name}
          </Badge>
        </div>
      )}
...
function BoardViewListButton({ isFixed }) {
  const qs = useQueryString();
  const { id } = useParams();
