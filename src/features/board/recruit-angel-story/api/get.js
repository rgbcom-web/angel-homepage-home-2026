...
export async function getList(tableName, searchParams, itemsPerPage) {
  try {
    ...
    data.map((item, i) => {
      ...
      if (!item?.thumbnail?.filePath) return;

      item.thumbnail.url = supa.storage
        .from(process.env.NEXT_PUBLIC_SUPABASE_STORAGE_BUCKET)
        .getPublicUrl(item.thumbnail.filePath).data.publicUrl;
    });
...
export async function getFixedList(tableName) {
  try {
    const supa = await createAdminClient();
