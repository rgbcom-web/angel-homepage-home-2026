 ...
 */
export async function uploadFileToStorage(validatedData, uploadPath) {
  ...
  try {
    ...
    if (toDeleteFiles.length > 0) {
      await supa.storage.from(BUCKET_NAME).remove(toDeleteFiles);
    }
    ...
    const uploadPromises = flattenedFiles.map(async ({ key, file }) => {
      ...
      const { data, error } = await supa.storage.from(BUCKET_NAME).upload(filePath, file);
